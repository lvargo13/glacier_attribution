...
dem_file='vertebrae25_10m.nc';  % needs to be in /nc_input/
rad_dem_file = 'vertebrae25_pdd.tif' ; 

% lat_glac = -42.889 ;  %roll
% lon_glac = 171.527 ;
...
% lon_glac = 168.238 ;
% lat_glac = -44.23 ;  % dainty/Essex
% lon_glac = 170.89 ;
% lat_glac = -43.683 ;  % stocking
% lon_glac = 170.074 ;
...
% figure;
% %plot(run_years+1,mmb,'o--'); hold on 
% plot(run_years+1,mmb_dateadj,'o--');hold on  % date adjusted MB
% xlabel('year'); ylabel('Mass Balance (mm w.e.)')
% plot(run_years+1,measmb,'o--')
%rmse = sqrt(nanmean((mmb_dateadj' - measmb).^2));

% % ELAs     
% sl = zeros(size(snowth,3),1); 
% le = zeros(size(snowth,3),1);
% % for i =1:length(sl)
% %    si = snowth(:,:,i);
% %    snowth_inds = find( (si>0) & (si<dc.SLThreshMax));
% %    le(i) = length(snowth_inds);
% %    
% %    if isempty(snowth_inds)
% %        if min(si(:)) > dc.SLThreshMax  % if min th is gt. max thres -> all snow
% %         sl(i) = min(dem(ice>0)); 
% %        else
% %         sl(i) = max(dem(ice>0)); 
% %        end
% %    else
% %       6  sl(i) = mean(dem(snowth_inds)); 
% %    end
% %    
% % end
% 
% for ii =1:length(sl)   
%    si = snowth(:,:,ii);
%    snowth_inds = find( (si>dc.SLThreshMin) & (si<dc.SLThreshMax));
%    
%    if isempty(snowth_inds)
%        if min(si(:)) > dc.SLThreshMax  % if min th is gt. max thres -> all snow
%         sl(ii,1) = min(dem(ice>0)); 
%         sl(ii,2) = min(dem(ice>0)); 
%         sl(ii,3) = max(dem(ice>0));
%        else
%         sl(ii,1) = min(dem(ice>0)); 
%         sl(ii,2) = min(dem(ice>0)); 
%         sl(ii,3) = min(dem(ice>0)); 
%        end
%    else
%         sl(ii,1) = min(dem(snowth_inds)); 
%         sl(ii,2) = mean(dem(snowth_inds)); 
%         sl(ii,3) = max(dem(snowth_inds)); 
%    end
%    
% end
% 
...
%SL = [1877 2047 1855 NaN 1873 1881 1870 1942 1922 NaN NaN 1851 1778 1881 1843 1942 1862 2052 2130 2028 1896 2080 1887 1910 1860 1945 1942 2070 1995 1920 2170 2070 1920 1920 1922 NaN NaN]; % dainty
